...
func _ready():
	# 注册，并加载数据
	var wznode = WZLib.get_by_path("Character/Face/00020000.img")
...
func create_animation(animation_name, wznode):
	...
	self.sprite_frames.set_animation_loop(animation_name, true)
	...
	if animation_name == "default":
		...
		texture.resource_name = "%s#0"%[animation_name, 0]
		self.sprite_frames.add_frame(animation_name, texture, 30)
	else:
